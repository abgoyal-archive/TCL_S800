...
#define STK_C_CUST_ALS_LEVEL    16
#define STK_C_CUST_I2C_ADDR_NUM 1
...
struct stk_alsps_hw {
    ...
    unsigned char   i2c_addr[STK_C_CUST_I2C_ADDR_NUM];  /*!< i2c address list, some chip will have multiple address */
    unsigned int    als_level[STK_C_CUST_ALS_LEVEL-1];  /*!< (C_CUST_ALS_LEVEL-1) levels divides all range into C_CUST_ALS_LEVEL levels*/
    unsigned int    als_value[STK_C_CUST_ALS_LEVEL];    /*!< the value reported in each level */
