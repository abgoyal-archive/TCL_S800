...
#else
#define APS_FUN  APS_ERR
#define APS_LOG  APS_ERR
#define APS_DBG  APS_ERR
#endif
#ifdef MT6516
...
static int32_t stk31xx_set_ps_cali(void);
#endif
...
struct stk31xx_priv {
    ...
    u16         als_level_num;
    u16         als_value_num;
    u32         als_level[STK_C_CUST_ALS_LEVEL-1];
    u32         als_value[STK_C_CUST_ALS_LEVEL];
...
#if (defined(STK_MANUAL_CT_CALI) || defined(STK_MANUAL_GREYCARD_CALI))
static int32_t stk31xx_get_ps_cali_thd(int16_t *ps_thdh, int16_t *ps_thdl);
#endif
#if (defined(STK_MANUAL_CT_CALI) || defined(STK_MANUAL_GREYCARD_CALI) || defined(STK_AUTO_CT_CALI_SATU) || defined(STK_AUTO_CT_CALI_NO_SATU))
...
/*----------------------------------------------------------------------------*/
int stk31xx_get_addr(struct stk_alsps_hw *hw, struct stk31xx_i2c_addr *addr)
{
	if(!hw || !addr)
	{
		return -EFAULT;
...
/*----------------------------------------------------------------------------*/
static void stk31xx_power(struct stk_alsps_hw *hw, unsigned int on) 
{
...
int sitronix_ts_permission_thread(void *data)
{
	int ret = 0;
	int retry = 0;
...
static int32_t stk31xx_get_ps_cali_file(char * r_buf, int8_t buf_size)
{
	...
	if(IS_ERR(cali_file))
	{
        APS_ERR("%s: filp_open error!", __func__);
		return -ENOENT;
	}
	else
	{
		...
		if(ret < 0)
		{
			APS_ERR("%s: read error, ret=%d", __func__, ret);
...
static int32_t stk31xx_set_ps_cali_file(char * w_buf, int8_t buf_size)
{
	struct file  *cali_file;
	...
	else
	{
		...
		if(ret != sizeof(w_buf))
		{
			APS_ERR("%s: write error!", __func__);
			filp_close(cali_file,NULL);
			return -EIO;
		}
		...
		if(ret < 0)
		{
			APS_ERR("%s: read error!", __func__);
			filp_close(cali_file,NULL);
			return -EIO;
		}		
		set_fs(fs);
		
		//APS_LOG("%s: read ret=%d!", __func__, ret);
		for(i=0;i<STK_CALI_FILE_SIZE;i++)
		{
			if(r_buf[i] != w_buf[i])
			{
				APS_ERR("%s: read back error!", __func__);
				filp_close(cali_file,NULL);
				return -EIO;
			}
		}
...
static void stk31xx_set_ps_thd_to_file(int16_t ps_thd_data[])
{
	...
	stk31xx_set_ps_cali_file(w_buf, STK_CALI_FILE_SIZE);			
}
#endif	/*	#if (defined(STK_MANUAL_CT_CALI) || defined(STK_MANUAL_GREYCARD_CALI))	*/
...
{	
	...
	{
		APS_ERR("Min PS data:%d\n", ps_stat_data[STK_DATA_MIN]);	
		APS_ERR("PS reading is too small\n");
		return -2;				
	}
...
static int32_t stk31xx_judge_ct_cali_nosatu(int16_t ps_stat_data[])
{
	int16_t diff_ps_data_ct = 0;
	...
	{ 
		...
		if (diff_ps_data_ct > STK_MAX_PS_DIFF_AUTO)		
		{
			APS_ERR("Max PS data of crosstalk:%d, Max_crosstalk - Min_crosstalk:%d\n", ps_stat_data[STK_DATA_MAX], diff_ps_data_ct);	
			APS_ERR("The difference between Max PS code and Min PS code is too large\n");
			return -1;
		}
	}
	...
	{
		APS_ERR("Min PS data of crosstalk:%d is larger than ps_code_low_thd:%d\n", ps_stat_data[STK_DATA_MIN], ps_code_low_thd);	
...
static int32_t stk31xx_judge_ct_cali_c_thd_dur_en(int16_t ps_stat_data[], int16_t ps_thd_data[])
{
	int16_t diff_ps_data_ct = 0;	
	if(ps_stat_data[STK_DATA_MAX] < STK_MAX_PS_CROSSTALK)
	{
		...
		if (diff_ps_data_ct > STK_MAX_PS_DIFF_AUTO)		
		{
			APS_ERR("STK PS : Max PS data of crosstalk:%d, Max_ct - Min_ct:%d\n", ps_stat_data[STK_DATA_MAX], diff_ps_data_ct);	
			APS_ERR("STK PS : The difference between Max PS code and Min PS code is too large\n");
			return -1;
		}
	}
	else
	{
		APS_ERR("STK PS : Max PS data of crosstalk:%d\n", ps_stat_data[STK_DATA_MAX]);	
		APS_ERR("STK PS : PS crosstalk is too large\n");
		return -2;				
	}		
	...
	if(ps_thd_data[STK_HIGH_THD] > STK_THD_MAX || ps_thd_data[STK_LOW_THD] > STK_THD_MAX) 
	{
		APS_ERR("STK PS %s: threshold is too large!\n", __func__);
		return -11;
	}	
	
	if(atomic_read(&stk31xx_obj->ps_high_thd_val) < ps_thd_data[STK_HIGH_THD])
		return -3;
	return 0;	
}
#endif
...
static int32_t stk31xx_judge_ct_cali_satu(int16_t ps_stat_data[])
{
	int16_t diff_ps_data_ct = 0;
...
static int32_t stk31xx_get_several_ps_data(int16_t ps_stat_data[])
{
	...
	/*
	if((err = stk31xx_enable_ps(stk31xx_obj->client, 1)))
	{
	...
	*/
	...
	while(data_count < STK_CALI_SAMPLE_NO)
	{
		...
		{
			/*
			if((err = stk31xx_enable_ps(stk31xx_obj->client, 0)))
			{
				APS_ERR("enable ps fail: %1d\n", err); 
				return err;
			}
...
static int32_t stk31xx_set_ps_cali(void)
{
	...
	ret = stk31xx_judge_ct_cali_satu(ps_statistic_data);
	if(ret == 0)						
	{		
		ret = stk31xx_calc_ct_cali_thd(ps_statistic_data, ps_thd_data);
		if(ret < 0)
		{
		...
		stk31xx_set_ps_thd_to_file(ps_thd_data);			
	...
	if(stk31xx_obj->redetect_ct_en)
	{
		ret = stk31xx_judge_ct_cali_c_thd_dur_en(ps_statistic_data, ps_thd_data);
		if(ret == 0)
			stk31xx_set_ps_thd_to_driver(ps_thd_data);
	}
	else
	...
	//		ps_thd_data[STK_LOW_THD] = STK_MAX_PS_CROSSTALK	- STK_THD_L_BELOW_MAX_CT;	
			ps_thd_data[STK_HIGH_THD] =  ps_statistic_data[STK_DATA_MIN] - STK_THD_H_BELOW_MAX_CT;
			ps_thd_data[STK_LOW_THD] = ps_statistic_data[STK_DATA_MIN]	- STK_THD_L_BELOW_MAX_CT;
			stk31xx_set_ps_thd_to_driver(ps_thd_data);	
		}
		//else
			//APS_ERR("%s: calibration fail, errno=%d\n", __func__, ret);	
	...
	if(stk31xx_judge_ct_cali_nosatu(ps_statistic_data) == 0)
	{		
		ret = stk31xx_calc_ct_cali_thd(ps_statistic_data, ps_thd_data);
		if(ret < 0)				
		{
...
static ssize_t stk31xx_show_pscali(struct device_driver *ddri, char *buf)
{
	...
	ret = stk31xx_get_ps_cali_thd(&ps_thd_h, &ps_thd_l);		
	if(ret == 0)
	{		
		thd_auto[STK_LOW_THD] = ps_thd_l;
		thd_auto[STK_HIGH_THD] = ps_thd_h;	
...
#if defined(MTK_AUTO_DETECT_ALSPS)
	if((err = stk31xx_create_attr(&(stk31xx_init_info.platform_diver_addr->driver))))
#else
...
#if defined(MTK_AUTO_DETECT_ALSPS)    
	hwmsen_alsps_add(&stk31xx_init_info);
#else
