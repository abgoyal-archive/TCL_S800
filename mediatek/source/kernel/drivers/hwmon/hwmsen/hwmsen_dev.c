...
#define MAX_CHOOSE_ALSPS_NUM 2 //add,wanghai
//static char gsensor_name[25];
//static char msensor_name[25];
static char alsps_name[25]; //add,wanghai
...
static struct sensor_init_info* alsps_init_list[MAX_CHOOSE_ALSPS_NUM]= {0}; //wanghai
...
//./mediatek/source/kernel/drivers/hwmon/hwmsen
//===================================================
#if defined(MTK_AUTO_DETECT_ALSPS) //wanghai
...
printk("[hwmsen_alsps_add] %s\n", obj->name);
	...
	for(i =0; i < MAX_CHOOSE_ALSPS_NUM; i++ )
	{
	    if(NULL == alsps_init_list[i])
	    {
	      ...
	      alsps_init_list[i] = obj;//wanghai
//memcpy(alsps_init_list[i], obj, sizeof(struct sensor_init_info));
